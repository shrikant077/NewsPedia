...
const News = (props) => {
    ...
    // document.title = `NewsPedia - ${(props.category).charAt(0).toUpperCase() + (props.category).substr(1)}`;
    /*articles = [
      // {
      //     "source": {
      //         "id": "cnn",
    ...
    return (
        <div /*className='container my-3'*/>
            <h2 className='text-center'>NewsPedia - Trending {(props.category).charAt(0).toUpperCase() + (props.category).substr(1)} Headlines</h2>
            {loading && <Spinner vh="vh-100" vw="vw-50" />}
            <InfiniteScroll
                dataLength={articles?.length??0}
                next={fetchMoreData}
                hasMore={articles?.length !== totalResults}
                loader={loading && <Spinner vh="vh-40" vw="vw-50" />}
            >
            ...
            {/* <div className="container d-flex justify-content-between">
                    ...
                    <button disabled={page + 1 > (Math.ceil(this.totalResults / props.pageSize))} type="button" className="btn btn-primary" onClick={this.handleNextClick}>Next &rarr;</button>
                </div> */}
        </div>
    )
